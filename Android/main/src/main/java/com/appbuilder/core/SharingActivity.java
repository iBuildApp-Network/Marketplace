...
public class SharingActivity extends Activity implements
    ...
    private void initializeUI()
    {
        ...
        String stringToShare = currentIntent.getStringExtra("msg");
        editText.setText(stringToShare);
        editText.setSelection(stringToShare.length());
//        editText.setTextColor(getResources().getColor(R.color.black_trans_90));
//        editText.setHintTextColor(getResources().getColor(R.color.black_trans_60));
    }
    ...
    public void onClick(View arg0) {
        ...
        if (arg0.getId() == R.id.sharing_cancel) {
            closeCancel(-2,"");
        } else if (arg0.getId() == R.id.sharing_post) {
            showProgressDialog();
            if (sharingType.equalsIgnoreCase("facebook")) {
                new Thread(new Runnable() {
                    public void run() {
                        try {
                            String message_text = editText.getText().toString();// + " " + "http://ibuildapp.com/getmarket";//Statics.SHARING_URL;
                ...
                new Thread(new Runnable() {
                    public void run() {
                        try{
                            ...
                            // post message
                            StatusUpdate su = new StatusUpdate(editText.getText().toString());// + " " + "http://ibuildapp.com/getmarket");//Statics.SHARING_URL);
